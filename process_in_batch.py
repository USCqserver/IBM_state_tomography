...
for n in range(1,4):
    ...
    batch_list0 = [TomoData('ibmq_athens','20210625','MeasMainqFreeLong',0,p,s,runNo,i) for s in spec_pstates[1:] for i,p in enumerate(tetra_pstates)]
    batch_list1 = [TomoData('ibmq_athens','20210625','NonMarkMeasMainqFreeLong',0, p ,s ,runNo,i) for s in spec_pstates[1:] for i,p in enumerate(nonmark_pstates)]
    batch_list2 = [TomoData('ibmq_athens','20210625','SpecMeasMainqFreeLong',0,'tetra0', s ,runNo,i) for s in spec_pstates for i in range(5)]
    # batch_list3 = [TomoData('ibmq_athens','20210701','SpecRevMeasMainqFreeLong',0,'tetra0', s ,runNo,i) for s in spec_pstates for i,p in enumerate(tetra_pstates)]
    ...
    batch_list =  batch_list0 + batch_list1 + batch_list2
    ...
    for b in batch_list:
        ...
        if readout_error_mitigate and method == 'ibm':
            datapath = datapath.replace('raw', 'mem')
    ...
    for b in batch_list:
        ...
        # store data to
        filepath = r"/home/haimeng/LocalProjects/IBM-PMME/Analysis/" + exp0.device + "/" + exp0.datestr + '/' +runname + '/' + runNo+'/' + fittype + '/'
